...
func sync(population,at: Vector2,is_paused: bool) -> void:
	...
	for s in population.visible_sites(at):
		...
		for a in s.animals:
			...
			_entity(key,boar_mesh if a.species=="woodland_boar" else animal_mesh,center+Vector2(a.x,a.z))
			moving[key].set_meta("life_key","wild:"+id+":"+str(int(a.id)))
		for v in s.visitors:
...
func _scenery(s: Dictionary) -> Mesh:
	...
	if s.id not in ["wetland","riverbank"]:
		var spring=c+Vector2(-4,3); var y=terrain.ground(spring)
		G.cube(st,Vector3(spring.x,y+0.025,spring.y),Vector3(1.5,0.045,1.1),Color("70bbca"))
		for x in [-0.9,0.9]: G.cube(st,Vector3(spring.x+x,y+0.1,spring.y),Vector3(0.3,0.2,1.3),Color("9dac9b"))
	if s.camp:
...
func _process(dt: float) -> void:
	...
	for n in moving.values():
		...
		if ranch!=null and n.has_meta("life_key"):
			var task=ranch.lives.get(n.get_meta("life_key"),{}).get("task","")
			n.scale.y=0.5 if task=="休息" else 1.0
			n.rotation.x=lerpf(n.rotation.x,0.22 if task in ["吃草","饮水","吃饲料"] else 0.0,minf(1,dt*6))

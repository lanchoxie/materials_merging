...
static func query(v) -> Dictionary:
	...
	for dz in range(-1,2):
		for dx in range(-1,2):
			for tree in c.terrain.chunk_trees(chunk+Vector2i(dx,dz)):
				if at.distance_to(tree)>reach+0.4: continue
				var base=Vector3(tree.x,c.terrain.ground(tree),tree.y)
				var d=_distance(AABB(base-Vector3(0.2,0,0.2),Vector3(0.4,3,0.4)),eye,direction)
				if d>=0 and d<limit: limit=d; best={"type":"tree","name":"林木","distance":d,"point":eye+direction*d}
	...
	for id in v.world.regions:
		...
		for a in r.animals:
			...
			var spread=1.0 if r.enclosed else 2.2; var p=Vector2(center.x+a.x*spread,center.z+a.z*spread)
			if fish: p=Vector2(c.terrain.river_x(center.z+a.z*2.2)+a.x*0.4,center.z+a.z*2.2)
			if p.distance_to(at)>reach+1: continue
			var position=Vector3(p.x,c.terrain.ground(p),p.y)
			if fish: position.y=0.18+r.water*0.15
			var d=_distance(AABB(position-Vector3(0.3,0.1,0.5),Vector3(0.6,0.4 if fish else 0.95,1)),eye,direction)
			if d>=0 and d<limit: limit=d; best={"type":"animal","region":id,"id":a.id,"fish":fish,"health":a.health,"hunger":a.hunger,"name":("湿地鱼" if fish else "草甸小兽")+" #%d" % int(a.id),"distance":d,"point":eye+direction*d}
	return best
...
static func prompt(v,target: Dictionary) -> String:
	...
	match target.type:
		...
		"tree": return text+" · 树木保留，地上的枯枝可采集"
	return text

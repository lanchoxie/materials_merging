...
func input_error(key: String,reference: String,batch_id: String,c) -> String:
	if reference not in ["urea","water"]: return "这里仅接收已识别的尿素或水样；未知物性不会当成安全"
	...
	if reference=="urea" and t.solid_g+t.dissolved_g+float(rules.game.sample_urea_g)>float(rules.game.tank_solid_limit_g)+0.000001: return "已达到本罐的配料量上限；这不是溶解度上限"
	return ""
...
func add_input(key: String,reference: String,batch: Dictionary) -> void:
	...
	else: t.solid_g+=float(rules.game.sample_urea_g)
...
func bottle_error(key: String) -> String:
	...
	if t.get("dissolved_g",0)<=0.000001: return "先加入尿素，等待晶粒溶解"
...
func tick(field,c) -> void:
	for key in tanks.keys():
		...
		var delta=minf(t.solid_g,float(rules.game.dissolve_g_per_second))
...
func balance() -> Dictionary:
	...
	for row in inputs.values():
		if row.reference=="urea": input_g+=row.count*float(rules.game.sample_urea_g)
		else: input_l+=row.count*float(rules.game.sample_water_l)
...
func serialize() -> Dictionary:
	return {"version":1,"tanks":tanks.duplicate(true),"bottles":bottles.duplicate(true),"soils":soils.duplicate(true),"inputs":inputs.duplicate(true),"serial":serial,"spent_g":spent_g,"applied_water_l":applied_water_l,"waste_g":waste_g,"waste_l":waste_l}
...
func restore(data,c) -> bool:
	...
	for id in data.inputs:
		...
		if row.get("reference") not in ["urea","water"] or not _num(row.get("count"),1,1e9,true) or not row.get("work") is Dictionary: return false

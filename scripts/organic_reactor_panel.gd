...
func setup(model,index: int=-1) -> void:
	...
	var left=UI.box(self,Rect2(28,118,700,636)); var col=UI.column(left,14)
	col.add_child(UI.label("尿素 · 一片新叶",26,UI.MINT))
	col.add_child(UI.paragraph("CH₄N₂O · C ×1 / H ×4 / N ×2 / O ×1\n沿用所选炉里的同种元素，只补充缺少的位点。调整原子位置由你完成；博士负责把分子装进去。",17))
	contents=UI.paragraph("",16,UI.LILAC); col.add_child(contents)
	var box=SubViewportContainer.new(); box.custom_minimum_size=Vector2(650,295); box.stretch=true; col.add_child(box)
	var viewport=SubViewport.new(); viewport.size=Vector2i(650,295); viewport.own_world_3d=true; viewport.render_target_update_mode=SubViewport.UPDATE_ALWAYS; box.add_child(viewport)
	...
	col.add_child(UI.paragraph("反应釜完成的分子先留在炉内；收获后才成为可用样品。配料罐只混合背包里已有的尿素和水。",16))
...
func refresh() -> void:
	...
	stocks.text="背包尿素 %d份 · 本炉待收 %d份\n收获后，同一背包可供河湾配料罐取用" % [q.stock,q.pending]
	...
	harvest_button.visible=q.phase in ["producing","ready"]; harvest_button.disabled=q.pending<=0; harvest_button.text="收获尿素到背包 · %d份" % q.pending

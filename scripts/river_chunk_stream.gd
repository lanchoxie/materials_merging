...
func schedule(at: Vector2,new_season: int) -> void:
	...
	if center==center_key and new_season==season: return
	...
	pending=[]
	for key in wanted:
		if not chunks.has(key) or chunks[key].get_meta("season",-1)!=season: pending.append(key)
...
func drain(limit: int) -> void:
	for i in range(mini(limit,pending.size())):
		...
		chunks[key].mesh=mesh; chunks[key].set_meta("season",season); last_build_us=Time.get_ticks_usec()-start
...
func _build(key: Vector2i) -> Mesh:
	...
	for p in terrain.chunk_trees(key):
		var h=terrain.ground(p); var tall=2.2+(sin(p.x*4.1)+1)*0.45
		G.cube(st,Vector3(p.x,h+tall*0.45,p.y),Vector3(0.4,tall*0.9,0.4),Color("8c684d"))
		var leaf=Color(["497b5c","467a50","ca8c54","a0b9a5"][season])
		G.cube(st,Vector3(p.x,h+tall,p.y),Vector3(1.9,1.25,1.8),leaf)
		G.cube(st,Vector3(p.x+0.1,h+tall+0.85,p.y),Vector3(1.2,0.6,1.2),leaf.lightened(0.1))
